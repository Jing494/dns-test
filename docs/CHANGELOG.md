...
- 2026-08-26（第九十九轮）：**加固轮：突变检测前值0ms除零 + --since/--until locale 无关化（trends.sh / tests/06 / verify.sh / smoke.yml / docs，发布 v2026.08.26，语义版 v1.16A）**
  ...
  - **开放项复核结论**：#6（DNSUtil.pm `::` 满段宽松）非错误维持；#7（run_common_tests 体量）维持；trend_stats 的 `s/NR` 均值经查有 `n_ok<1` 早退保护，无除零风险（本轮排查确认，未改）
