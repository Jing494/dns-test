...
> 对应仓库：`dns-test`（MIT，当前版本 `v1.22 / v2026.09.4`）
...
│   ├── 06_compare_e2e.sh         # compare 端到端 132 用例（--watch参数校验/当前DNS👤标记三出口/环比Δ/提供商标签+抖动/预设组名展开/trends --prune/--until/--alert/--vs/周对比/突变检测(前值0ms边界)/--since当日边界/--md/--json/--week/--webhook/--archive 归档/--export 报障包/HTML归档小节/compare↔trends互链，mock dig/ping 离线 + 用户 results 备份恢复）
...
3. 单元测试（18+9+4+29+17+132+60+23+94+15 用例）
4. 冒烟测试（24 项/25 检查点）
5. compare 快测（2 DNS）
