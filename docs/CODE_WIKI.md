...
> 对应仓库：`dns-test`（MIT，当前版本 `v1.11 / v2026.08.18`）
...
│   └── 06_compare_e2e.sh         # compare 端到端 59 用例（--watch参数校验/当前DNS👤标记三出口/环比Δ/提供商标签+抖动/预设组名展开/trends --prune，mock dig/ping 离线 + 用户 results 备份恢复）
├── tools/                        # 专项测试工具
│   ├── manifest.sh               # 插件注册表
...
| [tests/06_compare_e2e.sh](file:///workspace/tests/06_compare_e2e.sh) | compare 端到端 59 用例（--watch 缺值/非法值/0 报错、当前系统 DNS 检测与 👤 标记三出口、环比 Δ 计算、提供商标签+抖动三出口+JSON jitter_ms、预设组名展开含 IPv6、未知词报错、trends --prune 保留/删除/校验；mock dig/ping 离线，用户 results/ 自动备份恢复） | `bash tests/06_compare_e2e.sh` |
