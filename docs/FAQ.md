...
- 结果对比（疑似劫持提示）扣分：CDN域名解析不同是负载均衡正常现象
...
bash smoke_test.sh        # 自动化冒烟 24 项/25 检查点，全绿 = 环境就绪
...
- **Windows**：不支持原生运行，推荐 **WSL**（一条命令开装：`wsl --install` → 装依赖 → 跑 smoke，完整步骤见上方"支持哪些操作系统"）；Git Bash 仅部分功能可用
