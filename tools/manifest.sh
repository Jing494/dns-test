...
# 字段（| 分隔）: 插件id | 脚本 | 菜单名称 | 执行器(perl/bash) | 引导提示(空=无) | 透传DNS(1=透传/0=不透传, 默认1) | 接受命令行参数(1=接受/0=不接受, 默认0)
...
PLUGIN_ITEMS=(
  ...
  "vowifi|carrier_epdg.pl|运营商ePDG部署检测|perl|运营商(ct/cmcc/cucc/cbn/all，输入直跑；回车进交互选择)|0|1"
