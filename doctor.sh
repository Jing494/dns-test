...
case "${1:-}" in
  ...
  --cron)
    ...
    echo ""
    echo "# 1) 每 30 分钟采集一轮并聚合（保留最近 200 份，清理前先归档到 trends/archive/）"
    echo "*/30 * * * * cd ${SCRIPT_DIR} && bash trends.sh --cron 223.5.5.5 119.29.29.29 --prune 200 --archive >> trends/cron.log 2>&1"
    echo ""
    ...
    echo ""
    echo "# 3) 每周日 03:00 全量归档（备份/迁移/报障分享，不删文件；归档包积多可定期清理）"
    echo "0 3 * * 0 cd ${SCRIPT_DIR} && bash trends.sh --archive >> trends/cron.log 2>&1"
    echo ""
