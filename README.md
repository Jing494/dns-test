...
![Release: v1.7.0](https://img.shields.io/badge/Release-v1.7.0-blue.svg)
![Version: v2026.08.10](https://img.shields.io/badge/Version-v2026.08.10-blue.svg)
...
#   下载 dns-test-v2026.08.10.tar.gz 后解压即可
...
- **单元测试（✅ 已完成）**：`lib/DNSUtil.pm` 提取 DNS 纯函数（9 个：sockaddr/域名编码/响应解析/PTR/反向名/IPv6 展开等）+ `tests/01_dnsutil.t` 18 用例（perl）+ `tests/02_plugins.sh` 9 用例（bash 轻量断言：插件注册表/参数策略/拦截）+ `tests/03_dig_target.sh` 4 用例（IPv6 加方括号）+ `tests/04_core_functions.sh` 13 用例（地址校验/响应判断/CDN 判定），9 个 perl 脚本全量迁移 DNSUtil，已接入 verify + CI strict；运行 `perl -Ilib tests/01_dnsutil.t` / `bash tests/02_plugins.sh` / `bash tests/03_dig_target.sh` / `bash tests/04_core_functions.sh`
