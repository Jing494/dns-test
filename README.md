...
> 🔧 **最新版本**：**v1.16 = v2026.08.24**（补丁轮：macOS 自带 bash 3.2 多字节字符展开损坏修复——trends 总图标题/`--vs` 对决行/时间窗倒挂报错在旧 bash 下的显示异常；`--open` 无桌面环境降级提示；CI 双平台加固）。v2026.08.23 收官轮带来 `--archive-keep` 归档轮转 + `doctor --fix` 自愈 + `--export` 时间窗 + trends 纯函数下沉 + compare↔trends 互链。完整版本历程（每轮）见 [docs/CHANGELOG.md](./docs/CHANGELOG.md)。
