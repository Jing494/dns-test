...
> 📍 **目录可自由放置**：脚本全部相对定位（`BASH_SOURCE`），拉取/复制到任意目录都能运行。文档示例中的 `/workspace/dns-test/` 路径请按你的实际目录替换。
...
脚本运行时会自动输出环境摘要（`🌐 环境: Linux/macOS | dig✅ perl✅ ping✅ | IPv6:可用 | 端口测试需真机`），测试结果可据此回溯。

| 测试类型 | 沙箱（AI代测） | 真机（自己跑） |
|---------|--------------|---------------|
| DNS 解析/对比/评分 | ✅ 完整 | ✅ 完整 |
| 运营商 ePDG 部署检测 | ✅ 完整 | ✅ 完整 |
| 路由器转发检测 | ⚠️ 需传路由器IP | ✅ 推荐（测自家网络） |
| IPv6 连通性 [7b] | ⚠️ 看沙箱网络 | ✅ 完整（家宽） |
| 端口连通性 | ⚠️ UDP出站受限 | ✅ 完整 |

> AI 在沙箱代测：DNS 类测试可信；端口/路由器类建议让用户真机自测。
