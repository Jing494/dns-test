...
for _d in "since:$SINCE" "until:$UNTIL"; do
  _dv="${_d#*:}"; _dn="${_d%%:*}"
  if [ -n "$_dv" ]; then
    if ! [[ "$_dv" =~ ^([0-9]{4})-([0-9]{2})-([0-9]{2})$ ]]; then
      echo "❌ --${_dn} 格式必须为 YYYY-MM-DD（例: 2026-08-01），收到: $_dv"
      exit 1
    fi
    # 语义校验：13月/99日等形状合法但非法的日期会静默过滤出空集，同样显式报错
    # （仅做月份/日期基本范围校验，不追求闰年/大小月的精确日历校验）
    _y="${BASH_REMATCH[1]}"; _m="${BASH_REMATCH[2]}"; _dd="${BASH_REMATCH[3]}"
    if [ "$_m" -lt 1 ] || [ "$_m" -gt 12 ] || [ "$_dd" -lt 1 ] || [ "$_dd" -gt 31 ]; then
      echo "❌ --${_dn} 不是有效日期（月须 01-12 / 日须 01-31），收到: $_dv"
      exit 1
    fi
  fi
done
